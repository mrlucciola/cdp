...
export const depositCollateralFAIL_NotEnoughTokens = async (user: User, accounts: Accounts) => {
  ...
  assert(depositAmount > ataBalPre,
  ...
  assert(diff == 0, "Deposit did not fail when trying to deposit more tokens than in the user's ATA");
};
...
export const depositCollateralPASS = async (user: User, accounts: Accounts) => {
  ...
  assert(userBalPre >= depositAmount,
    "Test requirest ATA balance to be >= deposit amount. Please increase deposit amount" +
    "ATA Balance: " + userBalPre + " Deposit Anount: " + depositAmount);
  assert(
  ...
  assert(Math.abs(depositAmount + userDiff) < differenceThreshold,
    "Expected User ATA Diff: " + (-depositAmount) + " Actual User ATA Diff: " + userDiff);
  assert(Math.abs(troveDiff - depositAmount) < differenceThreshold,
...
export const depositCollateralFAIL_DepositExceedingTVL = async (user: User, accounts: Accounts) => {
  ...
  assert(diff == 0, "Deposit failed but token balance changed after deposit attempt");
  ...
  assert(tvlPre.toNumber() - tvlPost.toNumber() == 0, "TVL changed after failed deposit when it should've stayed the same")
};
