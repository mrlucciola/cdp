// libraries
import { workspace, Program, utils, Wallet, Provider } from "@project-serum/anchor";
...
import { TokenAmount, Token as SToken, createInitMintInstructions, u64, SPLToken,} from "@saberhq/token-utils";
...
export class Accounts {
  ...
  public async initQuarry() {
    ...
    this.quarryProvider= new SignerWallet(this.quarryPayer)
    ...
    const { tx, mintWrapper: mintWrapperKey } = await this.quarrySdk.mintWrapper.newWrapper({
    ...
    const { tx: txRewarder, key: rewarderKey } = await this.quarrySdk.mine.createRewarder({
    ...
    [this.minterKey] = await findMinterAddress(mintWrapperKey, rewarderKey, QUARRY_ADDRESSES.MintWrapper);
