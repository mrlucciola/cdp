...
impl<'info> HarvestReward<'info> {
    pub fn harvest(&mut self) -> ProgramResult {
        ...
        let fee_info = calculate_fee(
            self.trove_reward.amount,
            self.global_state.fee_num,
            self.global_state.fee_deno,
        )?;

        require!(
            fee_info.new_amount > 0,
            StablePoolError::InvalidTransferAmount
        );
        require!(
            fee_info.owner_fee > 0,
            StablePoolError::InvalidTransferAmount
        );
...
impl<'info> BorrowUsdx<'info> {
    pub fn borrow(&mut self, amount: u64, user_usd_token_nonce: u8) -> ProgramResult {
        ...
        assert_vault_debt_ceiling_not_exceeded(
            self.vault.debt_ceiling,
            self.vault.total_debt,
            amount,
        )?;
        assert_global_debt_ceiling_not_exceeded(
            self.global_state.debt_ceiling,
        ...
        let signer_seeds = &[GLOBAL_STATE_SEED, &[self.global_state.global_state_nonce]];
        let signer = &[&signer_seeds[..]];
...
impl<'info> CreatePriceFeed<'info> {
    pub fn create_price_feed(&mut self, pair_count: u8) -> ProgramResult {
